...
import {
 BmLocalSearch, BaiduMap, BmView,BmMarker
} from "vue-baidu-map";
export default {
  data() {
    return {
  ...
  methods: {
    ...
    handleOk() {
      this.form.validateFields((err, values) => {
        ...
        const params = {
          ...values,
          latitiude:this.point.lat,
          longitiude:this.point.lng,
        };
        params["radius"] = parseFloat(values.radius) * 1000;
        console.info("新增区域代理信息：", params);
        this.$post("/agentInformation", { ...params }).then(() => {
      ...
      this.myGeo.getLocation(point, function (result) {
        if (result) {
          ...
          _this.form.setFieldsValue({
            agentinformationname:result.surroundingPois.length > 0
