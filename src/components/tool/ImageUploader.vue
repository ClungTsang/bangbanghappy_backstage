<template>
  <div>
    <!-- 图片上传组件 -->
    <a-upload accept="image/*" listType="picture-card" :fileList="fileList" :multiple="multiple" :remove="remove" :beforeUpload="beforeUpload" :customRequest="handleUpload" @preview="handlePreview">
      <!-- 达到上传最大值 -->
...
export default {
  ...
  methods: {
    ...
    // 删除图片
    remove(file) {
      // console.warn('remove_filelist', file)
      // console.log('origin_filelist', this.fileList)
      ...
      this.cos.putObject(
        ...
        (err, data) => {
          if (err) {
            ...
            // console.info('upload_fileList', this.fileList)
            this.$emit('upload_fileList', this.fileList)
          }
        }
      )
